...
class EntityA:
    def __init__(self, limit):
        ...
        self.t_increment = 20.0
        self.timer_running = False

    def output(self, message):
        if len(self.buffer) >= self.buffer_size:
            print("EntityA: Buffer full. Aborting.")
            sys.exit(0)
    ...
    def send_packets(self):
        while self.buffer and (self.next_seqnum < self.base + self.window_size):
            
            msg = self.buffer.pop(0)
            
           
            current_seq = self.next_seqnum % self.limit
            
            pkt = Pkt(current_seq, 0, 0, msg.data)
            pkt.checksum = self.calc_checksum(pkt)
            
            self.packets_in_transit[self.next_seqnum] = pkt
            
            to_layer3(self, pkt)
            
            if TRACE > 0:
                print(f"EntityA: Sent packet seq={current_seq} (Real seq: {self.next_seqnum})")
            
            if self.base == self.next_seqnum:
                if not self.timer_running:
                    start_timer(self, self.t_increment)
                    self.timer_running = True
            
            self.next_seqnum += 1

    def input(self, packet):
        checksum_ok = (packet.checksum == self.calc_checksum(packet))
        
        if not checksum_ok:
            if TRACE > 0:
                print("EntityA: Received corrupt packet. Ignoring/Waiting.")
            return

        # مدیریت NACK (طبق قرارداد قبلی: acknum=2 یعنی NACK)
        if packet.acknum == 2:
            if TRACE > 0:
                print("EntityA: Received NACK. Retransmitting window immediately.")
        ...
        ack_val = packet.acknum
        
        found = False
        potential_base = self.base
        
        for i in range(self.window_size + 5): 
            check_seq = (self.base + i) % self.limit
            if check_seq == ack_val:
                potential_base = self.base + i + 1
                found = True
                break
        
        if found:
            if TRACE > 0:
                print(f"EntityA: Received ACK for seq={ack_val}. Slide window to {potential_base}.")
            
            while self.base < potential_base:
                if self.base in self.packets_in_transit:
                    del self.packets_in_transit[self.base]
                self.base += 1
            ...
            if TRACE > 0:
                print(f"EntityA: Received duplicate/old ACK={ack_val}. Ignoring.")

    def timer_interrupt(self):
        self.timer_running = False
        
        if TRACE > 0:
            print(f"EntityA: Timer expired. Resending window from {self.base} to {self.next_seqnum-1}")
        
        start_timer(self, self.t_increment)
        self.timer_running = True

        for seq in range(self.base, self.next_seqnum):
            if seq in self.packets_in_transit:
                pkt = self.packets_in_transit[seq]
                to_layer3(self, pkt)
                if TRACE > 0:
                    print(f"EntityA: Re-sending packet seq={pkt.seqnum}")
    
    def calc_checksum(self, pkt):
        checksum = pkt.seqnum + pkt.acknum
        for byte_val in pkt.payload:
            checksum += byte_val
        return checksum
...
class EntityB:
    ...
    def input(self, packet):
        ...
        if not checksum_ok:
            if TRACE > 0:
                print("EntityB: Received corrupt packet. Sending NACK.")
            
            sndpkt = Pkt(0, 2, 0, self.dummy_payload) 
            sndpkt.checksum = self.calc_checksum(sndpkt)
            to_layer3(self, sndpkt)
            return

        if packet.seqnum == self.expected_seqnum:
            if TRACE > 0:
                print(f"EntityB: Received expected packet seq={packet.seqnum}. Delivering.")
            
            to_layer5(self, Msg(packet.payload))
            
            self.last_ack_sent = self.expected_seqnum
            self.expected_seqnum = (self.expected_seqnum + 1) % self.limit
            
            sndpkt = Pkt(self.last_ack_sent, self.last_ack_sent, 0, self.dummy_payload)
            sndpkt.checksum = self.calc_checksum(sndpkt)
            to_layer3(self, sndpkt)
            
        else:
            if TRACE > 0:
                print(f"EntityB: Out-of-order packet seq={packet.seqnum}. Expected {self.expected_seqnum}.")
            
   
            if self.last_ack_sent != -1:
                if TRACE > 0:
                    print(f"EntityB: Re-sending ACK for {self.last_ack_sent}")
                sndpkt = Pkt(self.last_ack_sent, self.last_ack_sent, 0, self.dummy_payload)
                sndpkt.checksum = self.calc_checksum(sndpkt)
                to_layer3(self, sndpkt)
            else:
                 if TRACE > 0:
                    print(f"EntityB: No packets received yet. Sending NACK.")
                 sndpkt = Pkt(0, 2, 0, self.dummy_payload)
                 sndpkt.checksum = self.calc_checksum(sndpkt)
                 to_layer3(self, sndpkt)
