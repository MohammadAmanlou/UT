...
// including headers ang libraries
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include "struct.h"
...
int main() {
	...
	Hall salon;
	Coupon takhfif;
	//to store the data we need txt files
	...
	// check the condition
		...
		int cheking_reapeted_users = 0;
		char* buffer;
		fflush(stdin);
		input(&std.username);
		int b = scanf("%lld", &std.student_id);
		input(&std.password);
		// check every word and compare
		while (!feof(users)) {
			input_file(&buffer, users);
			if (strcmp(buffer, std.username) == 0) {
				printf("The user already exist");
				cheking_reapeted_users++;
				break;
			}
		}
		if (cheking_reapeted_users == 0) {
			// if we dont already have the user
			printf("done");
