...
void UTax::trip(std::string origin, std::string destination, std::string passenger){
    if(have_trip(passenger, origin, destination)){
        for (int i=0 ; i<members.size();i++){
            if(members[i]->get_name() == passenger){
                if (members[i]->get_role() == "passenger"){
                    last_trip_id ++;
                    trips.push_back(new Trip(origin, destination, (Passenger*)members[i] , last_trip_id));
                }
