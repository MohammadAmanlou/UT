...
def generate_round_keys(key_text):
    key = BitVector(textstring=key_text)
    key = key.permute([i-1 for i in PC1])
    ...
    for shift in SHIFTS:
        ...
        right << shift
        combined = left + right
        round_key = combined.permute([i-1 for i in PC2])
        round_keys.append(round_key)

    return round_keys

def feistel(right, round_key):
    expanded = right.permute([i-1 for i in E])
    ...
    for i in range(8):
        ...
        output[i*4:(i+1)*4] = BitVector(intVal=val, size=4)

    return output.permute([i-1 for i in P])

def des_encrypt(plaintext, key_text):
    bv = BitVector(textstring=plaintext)
    bv = bv.permute([i-1 for i in IP])

    left, right = bv.divide_into_two()
    round_keys = generate_round_keys(key_text)
    ...
    final = right + left
    final = final.permute([i-1 for i in IP_INV])

    return final.get_bitvector_in_hex()
